...
echo "══ 段7 回歸(:80 vault 健康點、:8082 跳轉站;CT203 本機視角——CT260→dmz 僅放行 :443) ══"
vh=$(ssh pve24 "sudo pct exec 203 -- curl -sf -m 5 http://127.0.0.1/health" || true)
[ "$vh" = "OK" ] || { echo "✗ 回歸失敗::80/health=「$vh」"; exit 1; }
r82=$(ssh pve24 "sudo pct exec 203 -- curl -s -o /dev/null -m 5 -w '%{http_code}' http://127.0.0.1:8082/" || true)
