#!/bin/bash
# finish-portal-051.sh — 部署 portal:0.5.1(大廳新增「常用網站」板塊:Homepage bookmarks
#                        5 組 14 站搬遷,Homepage 退役儀式前置;src/sites.ts 為站點 SoT)
# 純 portal 改動,無新 env 鍵、無防火牆變更。
# 冪等:重跑無害。回滾:compose 蓋回 _backups/docker-compose.yml.before-portal050-<最早TS>
#       + docker compose up -d --force-recreate portal(0.5.0 image 仍在本機)。
set -euo pipefail
...
echo "== 3. CT201:sha 驗證 + 首跑備份 + 換源 + build 0.5.1 =="
...
# 首跑才建 before-051 備份(重跑時源已是 0.5.1,再備份會污染回滾點)
ls /root/_backups/portal-src-before-051-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-src-before-051-$TS.tgz static bff
...
rm -f /tmp/portal-static.tgz /tmp/portal-bff.tgz
docker build -q -t portal:0.5.1 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 →0.5.1(先驗後裝) =="
...
if "portal:0.5.1" not in s:
    assert "    image: portal:0.5.0" in s, "錨點未命中:image tag(現役非 0.5.0?)"
    s = s.replace("    image: portal:0.5.0", "    image: portal:0.5.1", 1)
...
ls /opt/monitoring/_backups/docker-compose.yml.before-portal050-* >/dev/null 2>&1 \
  || cp -a /opt/monitoring/docker-compose.yml /opt/monitoring/_backups/docker-compose.yml.before-portal050-$TS
...
echo "== 完成(TS=$TS)。驗收:大廳底部 SITES·常用網站 5 組 14 站(PC40 :8088 或 portal.hl) =="
echo "回滾:compose 蓋回 _backups/docker-compose.yml.before-portal050-<最早TS> + up -d --force-recreate portal"
