#!/bin/bash
# finish-portal-061.sh — 部署 portal:0.6.1(借貸未結卡升級:筆數+待收/待還方向拆分
#                        金額+點開未結明細;資料源 hl-write-life 方向拆分已先行)
# 冪等:重跑無害。回滾:compose 蓋回 _backups/docker-compose.yml.before-portal061-<最早TS>
#       + docker compose up -d --force-recreate portal(0.6.0 image 仍在本機)。
set -euo pipefail
...
echo "== 3. CT201:sha 驗證 + 首跑備份 + 換源 + build 0.6.1 =="
...
ls /root/_backups/portal-src-before-061-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-src-before-061-$TS.tgz static bff
...
rm -f /tmp/portal-static.tgz /tmp/portal-bff.tgz
docker build -q -t portal:0.6.1 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 →0.6.1(先驗後裝) =="
...
if "portal:0.6.1" not in s:
    assert "    image: portal:0.6.0" in s, "錨點未命中:image tag(現役非 0.6.0?)"
    s = s.replace("    image: portal:0.6.0", "    image: portal:0.6.1", 1)
...
ls /opt/monitoring/_backups/docker-compose.yml.before-portal061-* >/dev/null 2>&1 \
  || cp -a /opt/monitoring/docker-compose.yml /opt/monitoring/_backups/docker-compose.yml.before-portal061-$TS
...
assert do[\\\"total\\\"] is None and do[\\\"items\\\"] is None, \\\"未認證竟露金額/明細\\\"
assert \\\"receivable\\\" in do and \\\"payable\\\" in do
print(\\\"redacted 形狀 ✓:\\\", {k: do[k] for k in (\\\"count\\\",\\\"receivable\\\",\\\"payable\\\")})\"
curl -s -m 8 -H \"Remote-User: e2e\" http://127.0.0.1:8088/api/life | python3 -c \"
import json,sys
d=json.load(sys.stdin); do=d[\\\"debts_open\\\"]
assert isinstance(do.get(\\\"items\\\"), list), \\\"認證後應有明細\\\"
print(\\\"authed 形狀 ✓: recv\\\", do[\\\"receivable\\\"], \\\"pay\\\", do[\\\"payable\\\"], \\\"items\\\", len(do[\\\"items\\\"]))\"
...
echo "回滾:compose 蓋回 _backups/docker-compose.yml.before-portal061-<最早TS> + up -d --force-recreate portal"
