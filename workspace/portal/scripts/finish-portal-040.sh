#!/bin/bash
# finish-portal-040.sh — 待辦49 M4:部署 portal:0.4.0(kiosk 輪播:/kiosk 頂層路由,
#                        三屏大廳→設備→安全 20s/屏、tap 三分區、reduced-motion 瞬切)
# 純前端+版本字串,無新 env 鍵、無防火牆變更。
# 冪等:重跑無害。回滾:compose 蓋回 _backups/docker-compose.yml.before-portal040-<最早TS>
#       + docker compose up -d --force-recreate portal(0.3.0 image 仍在本機)。
set -euo pipefail
...
echo "== 3. CT201:sha 驗證 + 首跑備份 + 換源 + build 0.4.0 =="
...
# 首跑才建 before-040 備份(重跑時源已是 0.4.0,再備份會污染回滾點)
ls /root/_backups/portal-src-before-040-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-src-before-040-$TS.tgz static bff
...
rm -f /tmp/portal-static.tgz /tmp/portal-bff.tgz
docker build -q -t portal:0.4.0 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 →0.4.0(先驗後裝) =="
...
if "portal:0.4.0" not in s:
    assert "    image: portal:0.3.0" in s, "錨點未命中:image tag(現役非 0.3.0?)"
    s = s.replace("    image: portal:0.3.0", "    image: portal:0.4.0", 1)
...
ls /opt/monitoring/_backups/docker-compose.yml.before-portal040-* >/dev/null 2>&1 \
  || cp -a /opt/monitoring/docker-compose.yml /opt/monitoring/_backups/docker-compose.yml.before-portal040-$TS
...
echo "回滾:compose 蓋回 _backups/docker-compose.yml.before-portal040-<最早TS> + up -d --force-recreate portal"
