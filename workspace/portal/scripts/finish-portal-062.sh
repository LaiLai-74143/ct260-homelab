#!/bin/bash
# finish-portal-062.sh — 部署 portal:0.6.2(借貸卡改淨額視圖:全體互抵淨額+逐人合併
#                        明細;不可互抵筆(外幣/物品/金額未填)以文字保可見,防假兩清)
# 冪等:重跑無害。回滾:compose 蓋回 _backups/docker-compose.yml.before-portal062-<最早TS>
#       + docker compose up -d --force-recreate portal(0.6.1 image 仍在本機)。
set -euo pipefail
...
echo "== 3. CT201:sha 驗證 + 首跑備份 + 換源 + build 0.6.2 =="
...
ls /root/_backups/portal-src-before-062-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-src-before-062-$TS.tgz static bff
...
rm -f /tmp/portal-static.tgz /tmp/portal-bff.tgz
docker build -q -t portal:0.6.2 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 →0.6.2(先驗後裝) =="
...
if "portal:0.6.2" not in s:
    assert "    image: portal:0.6.1" in s, "錨點未命中:image tag(現役非 0.6.1?)"
    s = s.replace("    image: portal:0.6.1", "    image: portal:0.6.2", 1)
...
ls /opt/monitoring/_backups/docker-compose.yml.before-portal062-* >/dev/null 2>&1 \
  || cp -a /opt/monitoring/docker-compose.yml /opt/monitoring/_backups/docker-compose.yml.before-portal062-$TS
...
echo "回滾:compose 蓋回 _backups/docker-compose.yml.before-portal062-<最早TS> + up -d --force-recreate portal"
