#!/bin/bash
# finish-portal-050.sh — 部署 portal:0.5.0(電力模塊 live:BFF power 查詢+前端 6 tiles
#                        +事件;遊戲頁 MCSM 入口卡;數據管線=pve24 finish-ups-metrics.sh)
# 純 portal 改動,無新 env 鍵、無防火牆變更。
# 冪等:重跑無害。回滾:compose 蓋回 _backups/docker-compose.yml.before-portal050-<最早TS>
#       + docker compose up -d --force-recreate portal(0.4.2 image 仍在本機)。
set -euo pipefail
...
echo "== 3. CT201:sha 驗證 + 首跑備份 + 換源 + build 0.5.0 =="
...
# 首跑才建 before-050 備份(重跑時源已是 0.5.0,再備份會污染回滾點)
ls /root/_backups/portal-src-before-050-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-src-before-050-$TS.tgz static bff
...
rm -f /tmp/portal-static.tgz /tmp/portal-bff.tgz
docker build -q -t portal:0.5.0 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 →0.5.0(先驗後裝) =="
...
if "portal:0.5.0" not in s:
    assert "    image: portal:0.4.2" in s, "錨點未命中:image tag(現役非 0.4.2?)"
    s = s.replace("    image: portal:0.4.2", "    image: portal:0.5.0", 1)
