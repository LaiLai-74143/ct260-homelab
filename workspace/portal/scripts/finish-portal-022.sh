#!/bin/bash
# finish-portal-022.sh — 待辦49:部署 portal:0.2.2(遊戲頁玩家數=MCSM 使用者級 API 接上)
#                        + force-recreate 使容器吃到 portal.env 新值(Kuma API key)
# 冪等:重跑無害。回滾:compose 蓋回 _backups/docker-compose.yml.before-portal022-<TS>
#       + docker compose up -d --force-recreate portal(0.2.1 image 仍在本機)。
set -euo pipefail
...
echo "== 3. CT201:sha 驗證 + 備份 + 換源 + build 0.2.2 =="
...
# 首跑才建 before-022 備份(重跑時 bff 已是 0.2.2,再備份會污染回滾點)
ls /root/_backups/portal-bff-before-022-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-bff-before-022-$TS.tgz bff
...
docker build -q -t portal:0.2.2 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 0.2.0→0.2.2(先驗後裝)+ force-recreate =="
...
if "portal:0.2.2" not in s:
    assert "    image: portal:0.2.1" in s, "錨點未命中:image tag"
    s = s.replace("    image: portal:0.2.1", "    image: portal:0.2.2", 1)
...
# 首跑才建 before-022 備份(重跑時現役 compose 可能已是 0.2.2)
ls /opt/monitoring/_backups/docker-compose.yml.before-portal022-* >/dev/null 2>&1 \
  || cp -a /opt/monitoring/docker-compose.yml /opt/monitoring/_backups/docker-compose.yml.before-portal022-$TS
...
echo "== 完成。回滾點:各取「最早一份」/opt/monitoring/_backups/docker-compose.yml.before-portal022-* + /root/_backups/portal-bff-before-022-*.tgz(功能回滾走 compose 換回 0.2.1 image 即可) =="
