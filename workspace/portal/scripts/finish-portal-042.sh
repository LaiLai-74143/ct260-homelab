#!/bin/bash
# finish-portal-041.sh — 部署 portal:0.4.2(設備/安全頁 Grafana 圖表入口卡 +
#                        switch3f 卡接 SNMP 數據:sysUpTime/埠 up 數,待辦25 接入)
# 純 portal 改動,無新 env 鍵、無防火牆變更。
# 冪等:重跑無害。回滾:compose 蓋回 _backups/docker-compose.yml.before-portal042-<最早TS>
#       + docker compose up -d --force-recreate portal(0.4.1 image 仍在本機)。
set -euo pipefail
...
echo "== 3. CT201:sha 驗證 + 首跑備份 + 換源 + build 0.4.2 =="
...
# 首跑才建 before-042 備份(重跑時源已是 0.4.2,再備份會污染回滾點)
ls /root/_backups/portal-src-before-042-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-src-before-042-$TS.tgz static bff
...
rm -f /tmp/portal-static.tgz /tmp/portal-bff.tgz
docker build -q -t portal:0.4.2 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 →0.4.2(先驗後裝) =="
...
if "portal:0.4.2" not in s:
    assert "    image: portal:0.4.1" in s, "錨點未命中:image tag(現役非 0.4.1?)"
    s = s.replace("    image: portal:0.4.1", "    image: portal:0.4.2", 1)
...
ls /opt/monitoring/_backups/docker-compose.yml.before-portal042-* >/dev/null 2>&1 \
  || cp -a /opt/monitoring/docker-compose.yml /opt/monitoring/_backups/docker-compose.yml.before-portal042-$TS
...
echo "回滾:compose 蓋回 _backups/docker-compose.yml.before-portal042-<最早TS> + up -d --force-recreate portal"
