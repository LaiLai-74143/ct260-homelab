#!/bin/bash
# finish-portal-090.sh — 部署 portal:0.9.0(待辦50:生活頁「行程共享·帳號管理」面板)
#   新增 BFF /api/life/guest(GET list + POST op),Remote-User 閘門,轉發 CT260 life-chat /guest
#   → hl-guest svc → NocoDB People。前端生活頁 GuestPanel(僅 portal.hl 顯示):列出/新增/
#   改密碼/停用啟用/移除帳號。身分證字號與密碼雜湊儲存,前端不回顯。
# 前置:CT260 life-chat 已更新(含 /guest 路由)並重啟;hl-guest svc 就緒。portal.env 沿用既有
#   LIFE_CHAT_URL/LIFE_CHAT_TOKEN(無新增 env)。
# 在哪跑:CT260(codex@codex-ops)~/workspace/portal/scripts/。冪等:重跑無害。
# 回滾(兩層):tar -C /opt/portal -xzf /root/_backups/portal-src-before-090-<TS>.tgz
#   + compose 蓋回 _backups/docker-compose.yml.before-portal090-<最早TS> + up -d --force-recreate portal。
set -euo pipefail
...
grep -rq "行程共享" "$ROOT/frontend/dist/assets"/*.js \
  || { echo "bundle 無帳號管理面板字串,GuestPanel 沒進包?"; exit 1; }
...
echo "== 3. CT201:sha 驗證 + 首跑備份 + 換源 + build 0.9.0 =="
...
ls /root/_backups/portal-src-before-090-*.tgz >/dev/null 2>&1 \
  || tar -C /opt/portal -czf /root/_backups/portal-src-before-090-$TS.tgz static bff
...
rm -f /tmp/portal-static.tgz /tmp/portal-bff.tgz
docker build -q -t portal:0.9.0 /opt/portal/bff
echo build-OK
'"

echo "== 4. compose 換版 →0.9.0(先驗後裝) =="
...
if "portal:0.9.0" not in s:
    assert "    image: portal:0.8.2" in s, "錨點未命中:image tag(現役非 0.8.2?)"
    s = s.replace("    image: portal:0.8.2", "    image: portal:0.9.0", 1)
...
ls /opt/monitoring/_backups/docker-compose.yml.before-portal090-* >/dev/null 2>&1 \
  || cp -a /opt/monitoring/docker-compose.yml /opt/monitoring/_backups/docker-compose.yml.before-portal090-$TS
...
# 現役 static 含帳號管理面板
grep -rq \"行程共享\" /opt/portal/static/assets/*.js || { echo \"現役 bundle 無帳號面板,部署未生效?\"; exit 1; }
echo \"帳號管理面板已進包 ✓\"
...
echo "== 完成(TS=$TS)。驗收:portal.hl 生活頁底部見「行程共享·帳號管理」面板,可註冊/管理帳號 =="
echo "回滾(兩層):tar -C /opt/portal -xzf /root/_backups/portal-src-before-090-<TS>.tgz"
echo "          + compose 蓋回 _backups/docker-compose.yml.before-portal090-<最早TS> + up -d --force-recreate portal"
